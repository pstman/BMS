# Smart Building BMS (سیستم مدیریت هوشمند ساختمان)

A robust, multi-tenant Building Management System (BMS) integrated with Telegram and powered by Cloudflare Workers & D1 database.
یک سیستم چندمستاجری هوشمند برای مدیریت مجتمع‌های مسکونی و تجاری، یکپارچه‌سازی شده با تلگرام و زیرساخت ابری کلودفلر.

---

## 🚀 Features | قابلیت‌ها
- **Multi-Tenant Architecture:** Support for multiple buildings and independent units.
- **Telegram Webhook & Auth:** Secure phone number verification and automated user mapping.
- **Cloudflare Edge Compute:** Built on Cloudflare Workers and D1 (SQLite) for high-speed, global execution.

## 🛠️ Tech Stack | تکنولوژی‌های مصرفی
- Cloudflare Workers & D1 Database
- Telegram Bot API
- JavaScript (ES Modules)

## 👨‍💻 Author & Creator | طراح و سازنده
**Iman Barzjan (ایمان برزجان)**
